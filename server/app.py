...
# BusEventCreate moved to routers_projects.py (canonical home, used by
# make_bus_delivery_router). Moved in ms-127 e-4871 PR3c.
...
# POST /api/projects/{project_id}/bus (post_bus_event) moved to
# routers_projects.make_bus_delivery_router (ms-127 e-4871 PR3c).
...
# ms-127 e-4871 (PR3b+PR3c): bus delivery routes (send/receive/cursors/ack) moved to
...
app.include_router(
    _make_bus_delivery_router(
        ...
        _envelope_audit_view=lambda *a, **k: _envelope_audit_view(*a, **k),
        _envelope_nonce_store=lambda *a, **k: _envelope_nonce_store(*a, **k),
        _envelope_parent_lookup=lambda *a, **k: _envelope_parent_lookup(*a, **k),
        _fanout_bus_event=lambda *a, **k: _fanout_bus_event(*a, **k),
        _fanout_master_name_to_projections=lambda *a, **k: _fanout_master_name_to_projections(*a, **k),
        _find_bus_event_by_client_id=lambda *a, **k: _find_bus_event_by_client_id(*a, **k),
        _resolve_bus_event_user_ids=lambda *a, **k: _resolve_bus_event_user_ids(*a, **k),
        is_auth_enabled=lambda: _auth_enabled,
    )
)
