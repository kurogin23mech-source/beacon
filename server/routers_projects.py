...
# Mirror of app.py's module-level flag and bus-delivery constants.
# Needed by BusEventCreate (default value) and post_bus_event (verbatim body).
# ms-127 e-4871 PR3c: moved here together with the handler. The auth-enabled
# flag is NOT duplicated here — post_bus_event reads it via the injected
# ``is_auth_enabled()`` getter (same pattern as make_router / make_collab_router)
# so tests that flip app.py's shared ``_auth_enabled`` at runtime still take effect.
...
# Moved from app.py in ms-127 e-4871 PR3c (bus send write-path).
...
# ---------------------------------------------------------------------------
# Bus delivery router (ms-127 e-4871 PR3b + PR3c)
#
# Handles the bus send (write) path and the 6 synchronous "receive" side:
...
def make_bus_delivery_router(
    ...
    *,
    ...
    _envelope_audit_view: Callable,
    _envelope_nonce_store: Callable,
    _envelope_parent_lookup: Callable,
    _fanout_bus_event: Callable,
    _fanout_master_name_to_projections: Callable,
    _find_bus_event_by_client_id: Callable,
    _resolve_bus_event_user_ids: Callable,
    is_auth_enabled: Callable[[], bool],
) -> APIRouter:
    """Build the /api/projects/* PR3b+PR3c bus delivery slice.
    ...
    for _name, _dep in (
        ...
        ("_envelope_audit_view", _envelope_audit_view),
        ("_envelope_nonce_store", _envelope_nonce_store),
        ("_envelope_parent_lookup", _envelope_parent_lookup),
        ("_fanout_bus_event", _fanout_bus_event),
        ("_fanout_master_name_to_projections", _fanout_master_name_to_projections),
        ("_find_bus_event_by_client_id", _find_bus_event_by_client_id),
        ("_resolve_bus_event_user_ids", _resolve_bus_event_user_ids),
        ("is_auth_enabled", is_auth_enabled),
    ):
        if not callable(_dep):
            raise TypeError(
    ...
    @router.post("/api/projects/{project_id}/bus")
    async def post_bus_event(
        ...
        authoritative_sender_uid = (
            _auth_sub if (is_auth_enabled() and _auth_sub) else sender_uid
        )
