"""Go 版ビューワーと Python の盤 data が一致することの突き合わせ (ms-170 e-6362)。

なぜ要るか
----------
「Beacon が入っていない PC でも見られる」ために、盤の読み取りを Python と Go で
二重に持つことにした (SPEC KIGmqQTIanqUypbtmrTs の 2026-09-09 改訂)。Go 一本化
(e-6518) で **Python 素朴盤 (serve/PAGE) は撤去**され、Python 側に残るのは
``build_view`` (= ``beacon view --json`` の headless 経路 / この golden の生成元) だけ。
盤の描画は Go 版が唯一だが、その Go 盤と Python の ``build_view`` は同じ data から
同じ盤を出さねばならない。二重に持つ以上、**両者がズレていないことを機械で確かめ
続ける**のが唯一の歯止めになる。
