...
def test_gate_true_for_prod_default_and_empty_backend():
    assert commands._is_default_prod_backend("prod", "") is True
    assert commands._is_default_prod_backend("prod", "default") is True


def test_gate_false_for_other_backends():
    assert commands._is_default_prod_backend("prod", "aws-ga") is False
    assert commands._is_default_prod_backend("prod", "trailnode") is False


def test_gate_false_for_non_prod_environment():
    assert commands._is_default_prod_backend("staging", "") is False
    assert commands._is_default_prod_backend("", "default") is False
...
def test_marker_success_reports_updated():
    with mock.patch("subprocess.run", side_effect=_ok):
        r = commands._update_deployed_prod_marker("abc1234", json_mode=True)
    assert r["updated"] is True and r["error"] is None and r["rev"] == "abc1234"


def test_marker_empty_rev_is_noop():
    r = commands._update_deployed_prod_marker("", json_mode=True)
