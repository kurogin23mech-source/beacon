...
# ===========================================================================
# 3. Stale non-terminal declaration ⇒ unknown (方針1 + 判断4 固着 backstop).
# ===========================================================================

class TestStaleDeclarationIsUnknown:
    @pytest.mark.parametrize("state", [
        bus_liveness.STATE_RUNNING,
        bus_liveness.STATE_IDLE,
        bus_liveness.STATE_AWAITING_HUMAN,
    ...
    def test_stale_declaration_is_unknown_when_live(self, state):
        # 方針1: a live session whose last report is stale ⇒ server stamps unknown.
        now = _now()
        assert bus_liveness.derive_state(state, _stale(now), True, now, WINDOW) \
            == bus_liveness.STATE_UNKNOWN

    def test_stale_awaiting_human_not_live_is_unknown_not_frozen(self):
        # 判断4 固着 backstop: a dead session frozen in awaiting_human must NOT
        # keep nagging the inbox — it becomes unknown, not a lingering
        # awaiting_human.
        now = _now()
        assert bus_liveness.derive_state(
...
# ===========================================================================
# 5. Undatable declaration ⇒ treated as stale (safe side).
# ===========================================================================

class TestUndatableDeclaration:
    def test_declaration_without_timestamp_is_stale(self):
        # A non-terminal state we cannot date is untrustworthy ⇒ unknown.
        now = _now()
        assert bus_liveness.derive_state(
            bus_liveness.STATE_RUNNING, None, True, now, WINDOW
        ) == bus_liveness.STATE_UNKNOWN

    def test_declaration_with_unparseable_timestamp_is_stale(self):
        now = _now()
        assert bus_liveness.derive_state(
            bus_liveness.STATE_RUNNING, "not-a-date", True, now, WINDOW
        ) == bus_liveness.STATE_UNKNOWN
