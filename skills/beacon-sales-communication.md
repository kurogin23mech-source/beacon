...
**スレッド集約の原則 (e-3535)**: 活動に紐づける時は、そのやり取りが属する会話スレッドの
**起点となる既存の活動 (`act-`/`nrt-`)** を選ぶ。**新しい活動を作って散らさない** — 1 スレッド
の証跡は 1 活動配下に集約する。打診→確定→フォローが別々の活動に割れると、フェーズの後始末
(fold) や履歴が読めなくなる (opp-6 dogfood: 同一スレッドの証跡が別活動に分断された)。
紐づけ先が判然としない時だけ人に 1 問確認する。
...
このやり取りから **顧客について新しく分かった持続的な知見** (顧客の課題 / 勝ち筋 /
キーパーソン・意思決定 / 予算サイクル・決裁の勘所 など) があれば、その顧客(Account)の顧客
ドキュメント (dossier) に追記する。証跡(Communication)は「何が起きたか」、dossier は「顧客に
ついて何が分かっているか」の蒸留 — 別物なので両方に残す。
...
の追記経路に渡す。**出た情報を dossier の見出しに AI が振り分けて置き、その解釈を見せる**
(「この課題は現場の話として書きました」)。宿題を出すのでなく、引き出された情報を自然に
マッピングする。dossier が無ければ既定テンプレで新規作成。
