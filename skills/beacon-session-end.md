...
**作業した target への帰属 (ms-153 e-5550 → ms-164 e-5942 / SPEC 方針3)**: session log は「project 一本のまとめ」ではなく **実際に作業した target (= 対象)** に帰属付けて記録される (read=root / write=作業 target の非対称)。帰属先は CLI が自動で決める — **fork worktree では `.beacon/fork.json` の `target_ms_id` を土台に** (この fork がその target を進めるために立てられたため)、session 中の commit が載った target 群と **合わせて** 帰属する。ms-164 で帰属は **複数化 (multi-attribution)**: 1 つの session が 1 日で複数 target を横断して触れても、その **全ての作業 target に帰属** する (旧挙動は複数横断を「曖昧」として未帰属に畳んでいた)。これにより 1 つの session log を root からも各子 target からも辿れる。commit も fork も無いときだけ active target にフォールバックし、それも無ければ未帰属 (project 全体) のまま。Skill 側で target を指定する必要はない (payload の `target_ids` (複数) / 後方互換の先頭 `target_id` / `target_source` に自動記録)。判定規則そのものは occupation 側の単一ヘルパー `occupation.resolve_worked_targets` に集約されており、note / push も同じ規則 (`resolve_worked_target_ids` wrapper 経由) を共有する。deploy は出荷 commit 由来の帰属を別に持ち (occupation.target_records で全 target-class に generic だが専用経路)、incident は明示した格上げ先を `occupation.resolve_target` で解決する (いずれも occupation 抽象経由で職種中立、ms-164 実装順序1〜2)。
