...
`calendar_namespace` / `calendar_account` / `tag` を持つ。この一覧が **突合の候補**。
...
終了に落とした面談は、次段 A (議事録取得→フェーズ遷移判定→次活動提案、e-3435) の入力。
**A はまだ未実装**なので、今は以下に留める (A が入ったらここが A 起動に置き換わる):

- status=ended の面談が A の処理キュー (= A は `beacon meeting ended` ではなく status=ended
  を対象に走る)。この Skill は二重に A を起動しない (終了済みは Step 1 の候補から外れる)。
- 報告で「終了ワークフロー待ち」として可視化し、ユーザーが手で議事録取得→次活動に進める
  導線を示す (A 実装までの暫定)。
