...
判定基準 — **追記する**のは組織構造 / 意思決定プロセス / 課題 / 予算サイクル / キーパーソン /
嗜好 など、**次の商談にも効く持続的な知見**。**追記しない**のは日程・宿題など一過性の連絡
事項 (それは Communication で足りる)。新しい知見が無ければこの Step は飛ばす。

追記する場合、対象商談 `$OPP` の `account_id` (= `$ACC`) を `beacon opportunity list --json`
で解決し、その顧客の dossier に足す。`/beacon-sales-dossier` の追記経路 (Step 4「追記」=
既存本文を保持したまま日時セクションを足す append-only) に渡す。dossier が無ければ新規
作成。要点だけを 1〜数行で書く (議事録の丸写しはしない = 蒸留)。
