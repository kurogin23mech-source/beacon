...
**「機構を書いた」≠「達成した」チェック（done先行の防止 / CORE doc `task-done-judgment-principle` 原則6）**:
AC が「実世界の状態」や「全域 / 両方 / 常に / すべて」を約束している時、コードが在るだけで DONE にしない。**掃除機を買った ≠ 掃除した**。以下を必須確認し、満たさなければ DONE でなく △ PARTIAL（機構は done、実行・配線は follow-up として切り出す）:
- **呼び出し確認（定義でなく使用を見る）**: 新規の関数/機構が production 経路から実際に呼ばれているか `git show <hash> --stat` + `grep -rn <name> lib/ server/ bin/` で確認。呼び出し元がテストだけなら未配線 = PARTIAL。
- **最終状態確認（関数の存在でなく実行結果）**: AC が「データが移し替え済」等の状態を約束するなら、その状態が現に成立しているか確認。実行していない/確認手段が無いなら PARTIAL。
- **「全域/両方/常に/すべて」を含む AC は反例を 1 つ探す**: 直読み等が 1 箇所でも残れば「全域」は未達。1 つでも見つかれば PARTIAL/SKIP。
...
5. **「機構を書いた」で done にしない**。関数/機構を書いても production で呼ばれ実行され AC の最終状態に到達していなければ △ PARTIAL（done先行の防止、原則6）。
...
## Step 4.7: 判断の自己申告 — log-time decision backstop (ms-154 e-5594)

コミットは「結果」だけを残し、「なぜその選択をしたか」という **非自明な判断** を落とす。決定を確実に捕獲する forcing function (= 決定点で宣言を強制する仕組み) の本命は重いので、まず **commit→log のこの地点で AI が自己申告する backstop (= 最後の受け皿)** を置く。SPEC `0iYyU79MEsxN4wGY7ADk` §設計方針3。

### adversarial に問う (過少報告バイアス対策)

このコミットの diff と commit message を振り返り、自分に **敵対的に** 問う:

- この変更で、**別の合理的な選択肢があったのに一方を選んだ** 箇所はどこか？
- 別の開発者・別の AI なら **違う判断をしたかもしれない** 点はどこか？
- 「却下した案」「先送りにした対応」「あえて狭くしたスコープ」はあるか？

AI は自分の生成を後から合理化しがちで、判断を「自明だった」と過少申告する傾向がある。**「特に無い」で終える前に、上の3問に具体的に答えられないか一度探す**。

### 非自明フィルタ (ノイズ防止)

以下は **記録しない** (自明 / 機械的):
- 誤字修正・フォーマット・変数リネーム等、選択の余地が無い変更
- SPEC やタスク AC がその選択を既に明示していて、実装が忠実に従っただけの場合
- 1 つの明白な正解しかない変更

記録する (非自明):
- 設計上のトレードオフを選んだ (例: 「新設せず既存 field を再利用」「専用 builder を置かず汎用口に寄せた」)
- レビュー指摘や案を **却下 / 採用** した
- 対応を **先送り / スコープ外** に切り出した
- 後方互換・性能・安全性のために一方に倒した

### 記録する

非自明な決定があれば、1 件ずつ Bash ツールで記録する (= 統一 decision arm に載せる):

```bash
cd "$PROJECT_DIR" && beacon decision record \
  --what "<何を選んだか — 1 行>" \
  --rationale "<なぜ — 却下した代案も 1 句添える>" \
  --evidence "commit:<hash:7>" --evidence "<file:line など根拠があれば>" \
  --kind log-backstop --decided-by autonomous-AI \
  --related-task "<関係する e-XXX があれば>"
```

- `--decided-by` は既定 `autonomous-AI` (= 人間未確認の AI 単独判断が最も監査が要る)。人間が選んだ決定なら `AI-proposed-human-chose` 等に変える。
- `--evidence` は複数回指定可。一級決定 (decided_by を立てる) は **根拠 link 必須** — 最低でも `commit:<hash>` を付ける。
- local mode (= cloud 未接続) では decision stream が無く、コマンドは「記録しません」と表示して正常終了する。記録できなくても log フロー自体は止めない (= best-effort)。

非自明な決定が本当に無ければ、このステップは黙って飛ばす (= 過剰記録もノイズ)。判断軌跡は Step 4 報告に 1 行添えてよい (例: 「decision 記録: 2 件 (schema 再利用 / 専用 builder 却下)」)。
