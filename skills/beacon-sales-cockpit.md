...
商談ごとに、以下の優先順位で「次の一手」を 1 つ決める（上から評価し最初にヒットしたもの）:

1. **遷移日が未設定（非終了フェーズ）** → 最優先で「遷移日を置け」。フェーズのゴール判定日
   が無いと準備も追い込みも駆動できない（SPEC §2）。→ `/beacon-sales-schedule` で面談を
   確定すると遷移日も同時に入る。
2. **遷移日が超過（overdue）** → 「判定せよ」。`/beacon-sales-meeting-wrap`（議事録あれば）
   or `beacon opportunity judge` の 3 択に繋ぐ。
...
```
🎯 今日やること (営業コックピット)
  ⚠ 判定 [opp-3] ○○社（提案準備 / 遷移日 07-14 超過）
     → 議事録あれば /beacon-sales-meeting-wrap、無ければ judge の3択
  ⏰ 遷移日を置く [opp-7] △△社（商談準備 / 遷移日 未設定）
     → /beacon-sales-schedule で初回面談を確定（遷移日も同時に入る）
...
- **遷移日未設定を最優先で促す**（engine 駆動の起点、SPEC §2）。
