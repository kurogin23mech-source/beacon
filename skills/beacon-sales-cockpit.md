...
- dossier が**無い**、または **直近の面談/やり取りより明らかに古い**（更新が追いついて
  いない）Account を最大 2〜3 件だけ拾う。全件は出さない（毎日同じ顔ぶれで催促しない）。
- 提示は「今日やること」の**下に一段下げて**、低優先の気づきとして添える:

```
🗂 顧客資産の深掘り余地（任意）
  顧客○○社: 商談 opp-3 が提案準備まで進んでいるが dossier が薄い
    → 次の面談で意思決定プロセス/キーパーソンを確認し /beacon-sales-dossier に追記
```

「今日やるべき」商談アクションを押しのけない（あくまで下段の任意提案）。ここも AI が
**提案するだけ** — 追記自体は面談振り返り時の自動蓄積か、ユーザーが選んで dossier Skill で行う。
