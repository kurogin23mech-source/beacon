...
顧客ドキュメントは真っ白なメモ帳ではなく、**「顧客について分かったこと」をこういう観点
(課題 / 勝ち筋 / キーパーソン / 購買の勘所 等) で整理する見出しのついた形**で持つ (中身は
自由記述のまま、整理の観点だけ用意する)。toB (法人営業) の既定テンプレ (本格的な顧客攻略
プランは重すぎるので考え方だけ薄く写す。見出しは**前提知識が要らない言葉**にする):
...
2. 既存本文を取得 (Step 3 の `doc show`)。**無い場合 (新規)** は Step 3.5 のテンプレを
   `$ACC_NAME` で埋めた形から始める (先頭の見出しが一覧上のタイトルになる)。
3. 反映する (AI が振り分ける):
