...
> **帰属の realize 方法 (スコープ境界)**: 本 MS では「まとめ本文の保存先そのもの (= `session_logs` サブコレクション) を作業 target のレコード配下へ物理移動する」ことはしない。保存先の移動は schema 移行に当たり、方針1 の in-place strangler 原則 (= 既存データ無変更) のスコープ外だからである。代わりに session log レコードに `target_id` / `target_source` を付けて「どの target を進めた session か」を構造的に辿れるようにする。`read=root / write=作業 target` の非対称のうち、write 側は現状「作業 target への帰属付け」まで realize されている (本文の物理再配置は後続候補)。
