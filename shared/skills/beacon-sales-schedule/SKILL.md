...
> 承認境界は **細粒度** (e-3694 / capability doc 原則4「合意事実の記帳は自動・承認は外向き
> 通信だけ」): 人が日時を確定した後の**記帳** (自分のカレンダーへの登録・仮押さえ解放・
> Meeting / 遷移日更新) は**自動**で進め、人間承認は**外向き通信** (相手への候補メール送信・
> 確定返信) だけに残す。面談確定スキル (e-3534) と同じ細粒度に揃える。
...
**この日時確定が人間の判断境界** (= 合意事実の確定, e-3694)。ここを人が言葉で決める以上、
Step 6 の記帳 (自分のカレンダー登録・Meeting/遷移日更新・仮押さえ解放) は改めて登録可否を
聞かず自動で進める。相手へ確定を伝える外向き通信だけは別途人間承認 (制約参照)。

## Step 6: 予定作成 (日時確定後は自動記帳) — 遷移日 + カレンダー + 識別 ID を束ねる

Step 5 で人が日時を確定したら、その記帳は**自動で進める** (改めて登録可否を聞かない,
e-3694)。登録するのは **自分のカレンダー** への予定 (相手に招待を送らない自己予約) と Beacon
の Meeting / 遷移日で、いずれも合意事実の記帳＝外向き通信ではない。何を登録したかは Step 8 で
報告する。以下を **この順で** 行う。ここが e-3433 (B) の核心 —
...
日時を直したい / やめる場合は、記帳が走る前 (= Step 5 の確定) で差し戻す — 直すなら Step 4
に戻り候補を出し直し、やめるなら中止する (確定していないので Meeting も作られない)。
...
- **承認境界は細粒度** (e-3694): 人が日時を確定 (Step 5) した後の記帳 (自分のカレンダー
  登録・仮押さえ解放・Meeting / 遷移日更新) は**自動**。人間承認は**外向き通信** (相手への
  候補メール送信・確定返信) だけに残す。**相手を招待に含める (invite を送る) 場合は外向き**
  なので、その時は承認を要する (既定の自己予約は invite を送らない)。
