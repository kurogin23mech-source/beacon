...
def _help_registry():
    ...
    return [
        ...
        {"command": "beacon attention", "flags": ["--all-projects", "--attention-only", "--scope", "--root", "--json"], "description": "Session roster grouped by root target (作業 target / 状態 / activity / 待機); --attention-only narrows to 要対応 (awaiting_human / blocked / terminated:failed); --scope self|team; --root <id> filters to one root target; --all-projects spans every project you belong to (ms-159)"},
