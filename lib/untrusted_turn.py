...
# ms-169 e-6297: the ONE definition of an untrusted-turn "source" record — the
# {event_id, sender, preview} the gate surfaces so the human judges WHICH DM put
# the turn in an untrusted context. Every producer (dm_untrusted.build_source, the
# inbox-hook fallback, _normalize_sources) builds records through make_source, so
# adding a field is a one-line change here instead of a 4-file synchronised edit
# where one miss drops the field silently.
SOURCE_KEYS = ("event_id", "sender", "preview")


def make_source(event_id: str = "", sender: str = "", preview: str = "") -> dict:
    """Build a canonical ``{event_id, sender, preview}`` source record.

    All three fields are coerced to ``str`` (a missing / None value becomes ""),
    so every record has exactly ``SOURCE_KEYS`` present — readers (the gate's
    renderer) can rely on the keys existing. This is the single constructor for
    the shape (ms-169 e-6297)."""
    return {
...
# ms-169 e-6298: arm() returns one of these status strings (not a bare bool) so a
# caller can tell WHY arming did or didn't take effect from the return value
# alone — a blank session key and a failed state write are different outcomes that
...
def _write_all(root: "str | Path", state: dict) -> bool:
    """Write the state map, pruning stale entries. Best-effort (never raises).

    Returns True when the write landed, False on any I/O failure. arm() uses the
    result to report ARM_WRITE_FAILED instead of a green "armed" light when the
    state could not actually be persisted (ms-169 e-6298 / e-6254); the other
    callers ignore it (a failed record_asked / vet / disarm only fails MORE
    conservatively)."""
    now = _now_iso()
    if now:
        pruned = {}
...
def _normalize_sources(sources, event_ids) -> list:
    ...
    for s in (sources or []):
        ...
        out.append(make_source(
            event_id=eid, sender=s.get("sender"), preview=s.get("preview")))
    ...
    for raw in (event_ids or []):
        ...
        if not eid or eid in seen:
            continue
        seen.add(eid)
        out.append(make_source(event_id=eid))
    return out
...
def arm(root: "str | Path", session_key: str, *, event_ids=None, sources=None,
    ...
    Use ``arm_succeeded(status)`` for the "is it armed now?" boolean; the distinct
    statuses let ``cmd_dm_show`` report an accurate arming light instead of a
    green "armed" that a silent write failure would have faked (e-6254).
    ...
    return ARM_OK if _write_all(root, state) else ARM_WRITE_FAILED
