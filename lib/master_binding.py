...
``EXPERIMENTAL = True`` がこの状態をコードとして明示しています。
...
``EXPERIMENTAL`` マーカー、および tests/test_master_binding_e3621.py の未配線を固定する
テストです。
────────────────────────────────────────────────────────────────────────────
...
# experimental マーカー (e-4360): 本モジュールは意図的に未配線であることを明示する。
# production 経路 (lib/commands.py / server/app.py) から呼ばれておらず、wire は ms-111 の
# linking go-live 時に行う。docstring 冒頭の注記と整合させること。True である限り
