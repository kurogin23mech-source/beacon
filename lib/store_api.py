...
class StoreApi:
    ...
    def invite_org_member(self, org_id: str, *, email: str,
        ...
        """Invite an org member via the cloud API (participation-only).
        ...
        auth / 403 / transport → ``RuntimeError`` (= 既存 cloud path と一致)。
        """
        try:
            return self._client.invite_org_member(org_id, email=email, role=role)
        except RuntimeError as e:
            if "404" in str(e):
                raise ValueError(str(e)) from e
            raise
