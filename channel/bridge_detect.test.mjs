...
test('ms-165: an alive bridge on MY canonical sid is NOT a competitor (restart hand-off)', () => {
  ...
  // session_id. Without the canonicalSid exclusion this forced a divergent mint → the
  // new bridge heartbeat under a fresh sid the CLI never resolves → silent
  // receive death. With canonicalSid set, my own session's bridge is excluded so I
  ...
  assert.equal(
    claimIsOtherAliveBridge(claim, { myPpid: 851, myPid: 94943, canonicalSid: 'sv-mine', pidAlive }),
    false,
  )
})
...
test('ms-165: an alive bridge on a DIFFERENT sid still forces a mint (concurrent bclaude)', () => {
  ...
  // different sid — a fresh mint is still correct, canonicalSid must not suppress it.
...
test('ms-165: empty canonicalSid preserves pre-ms-165 behaviour (any alive other bridge counts)', () => {
  const claim = { session_id: 'sv-x', pid: 5000, parent_pid: 4000 }
  const pidAlive = aliveSet(5000, 4000)
  assert.equal(
    claimIsOtherAliveBridge(claim, { myPpid: 851, myPid: 94943, canonicalSid: '', pidAlive }),
    true,
  )
})
...
test('ms-165: detectOtherAliveBridges drops my own-sid bridge, keeps a different-sid one', () => {
  ...
  try {
    ...
    const found = detectOtherAliveBridges({
      ...
      myPpid: 851, myPid: 94943, canonicalSid: 'sv-mine', pidAlive,
    })
...
test('ms-165: the legacy single-claim path also honours the canonicalSid exclusion', () => {
  // The legacy .beacon/bridge.json branch must not be a hole: a same-canonical-sid
  // alive claim there is my own restart too, and must be excluded.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bd-'))
  try {
    const legacy = path.join(dir, 'bridge.json')
    fs.writeFileSync(legacy, JSON.stringify({ session_id: 'sv-mine', pid: 87349, parent_pid: 1533 }))
    const pidAlive = aliveSet(87349, 1533)
    const excluded = detectOtherAliveBridges({
      bridgesDir: null, legacyPath: legacy,
      myPpid: 851, myPid: 94943, canonicalSid: 'sv-mine', pidAlive,
    })
    assert.deepEqual(excluded, [])  // my own restart claim in the legacy path is excluded
    // A different-sid legacy claim is still detected.
    fs.writeFileSync(legacy, JSON.stringify({ session_id: 'sv-other', pid: 5000, parent_pid: 4000 }))
    const kept = detectOtherAliveBridges({
      bridgesDir: null, legacyPath: legacy,
      myPpid: 851, myPid: 94943, canonicalSid: 'sv-mine', pidAlive: aliveSet(5000, 4000),
    })
    assert.deepEqual(kept.map(c => c.session_id), ['sv-other'])
  } finally {
    fs.rmSync(dir, { recursive: true, force: true })
  }
})
